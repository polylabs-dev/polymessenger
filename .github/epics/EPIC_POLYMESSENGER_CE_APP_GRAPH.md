...
Integrate the Cognitive Engine and App Graph into Q Messenger. This registers all 11 messenger modules (8 circuits + 3 graphs) into the Stratum module graph, wires 9 dependency edges plus 2 cross-graph QKit bridges (blind_relay, media_stream), and configures 3 CE meaning domains with noise filtering and 2 SME panels.
...
- [ ] **P1.1** — Implement `qmessenger_app_graph.fl` with 11 module definitions and `make_qmsg_module` helper
- [ ] **P1.2** — Wire 9 `EDGE_REQUIRES` dependency edges in `qmessenger_app_graph_register`
- [ ] **P1.3** — Implement `qmessenger_register_bridge_edges` (2 bridges: QKit blind_relay, QKit media_stream)
- [ ] **P1.4** — Add `find_qmsg_module_id` helper and `qmsg_app_graph_metrics` store
...
- [ ] **P3.6** — Implement `qmessenger_register_ce` orchestrator and `full_ce_pipeline_setup` golden test

### Phase 4: Integration & Validation

- [ ] **P4.1** — Verify all 11 modules resolve via `find_qmsg_module_id` after registration
- [ ] **P4.2** — Verify bridge edges connect to live QKit blind_relay and media_stream module graphs
...
1. `qmessenger_app_graph_register` produces a `CsrStorage` with exactly 11 nodes and 9 edges
2. `qmessenger_register_bridge_edges` adds 2 bridge nodes with `EDGE_BRIDGE_TO` edges
...
6. `qmessenger_register_ce` orchestrator returns true on successful full pipeline setup
...
| File | Description |
|------|-------------|
| `circuits/fl/qmessenger_app_graph.fl` | 11-module app graph, 9 edges, 2 bridges, golden tests |
| `circuits/fl/qmessenger_meaning.fl` | 3 CE domains, noise filter, 2 SME panels, orchestrator, golden tests |
